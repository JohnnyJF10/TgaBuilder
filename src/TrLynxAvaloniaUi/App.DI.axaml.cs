    ...
    public partial class App
    {
        ...
        private void AddCoreServicesToProvider(IServiceCollection services)
        {
            ...
            services.AddSingleton<IEyeDropper, EyeDropper>(sp => new EyeDropper(
                    new TrLynxLib.Abstraction.Color(0, 0, 0, 0)));
        ...
        private TrLynxLib.Abstraction.Color GetSystemAccentColor(IServiceProvider serviceProvider)
        {
            ...
            {
                return new TrLynxLib.Abstraction.Color(color.R, color.G, color.B, color.A);
            }
            else
            {
                return new TrLynxLib.Abstraction.Color(128, 128, 128, 128); // Fallback to a default accent color (gray)
            }
        }
        ...
        private void AddViewVMsToProvider(IServiceCollection services)
        {
            ...
            services.AddSingleton(sp => new ModificationsOutViewModel(
                mediaFactory: sp.GetRequiredService<IMediaFactory>(),
                modificationHelper: sp.GetRequiredService<IModificationsHelper>(),
                bitmapOperations: sp.GetRequiredService<IBitmapOperations>()));

            services.AddSingleton(sp => new ModificationsInViewModel(
                mediaFactory: sp.GetRequiredService<IMediaFactory>(),
                modificationHelper: sp.GetRequiredService<IModificationsHelper>(),
                bitmapOperations: sp.GetRequiredService<IBitmapOperations>(),
                modificationOutViewModel: sp.GetRequiredService<ModificationsOutViewModel>()));

            services.AddTransient(sp => new BasicViewModel(
                modificationHelper: sp.GetRequiredService<IModificationsHelper>(),
                modificationInVM: sp.GetRequiredService<ModificationsInViewModel>()));
